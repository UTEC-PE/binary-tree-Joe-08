...
void insertIntoTree(Arbol &tester);
void removeFromTree(Arbol &tester);
void sortAndPruneHelper();
bool testTreeCompletion(Arbol* tester);
void print(Arbol* tester);
...
void insertIntoTree(Arbol &tester) {
    ...
    tester.insertar(numberToInsert);
}
...
bool testTreeCompletion(Arbol* tester) {
    int i = 0;
    for (iterador it = tester->begin(); it != tester->end(); ++it) {
        if (*it != helper.at(i)) {
            return false;
        }
        i++;
